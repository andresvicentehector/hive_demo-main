...
import 'package:sunboards_vuit/screens/update_screen.dart';
...
class EditPresas extends StatefulWidget {
  ...
  final int index;
  final Via via;
  const EditPresas({ required this.server, required this.presas, required this.index, required this.via});
...
class _EditPresas extends State<EditPresas> {
  ...
  Widget build(BuildContext context) {
    ...
    if(version=="25"){Pared= Wall25(sendMessage: _sendMessage, presas: widget.presas);}else {Pared=Wall15(sendMessage: _sendMessage, presas: widget.presas);
    ...
    return Scaffold(
      ...
      onPressed: ()  {
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) {
              connection?.dispose();
              return UpdateScreen(index: widget.index, via: widget.via, presas: widget.presas,);
            },
          ),
        );
      ...
      body: Container(
        ...
        decoration: BoxDecoration(image: DecorationImage(fit: BoxFit.contain ,image: AssetImage("images/fondo_trans15.png")) ),
        child:
